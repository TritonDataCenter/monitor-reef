...
// helper: generate an ed25519 private key in OpenSSH format (as produced by
// ssh-keygen -t ed25519) and return the raw key, its PEM encoding, and the
// colon-separated MD5 fingerprint.
func generateEd25519OpenSSHKey(t *testing.T) (ed25519.PrivateKey, []byte, string) {
	t.Helper()
...
func TestFormatPublicKeyFingerprint(t *testing.T) {
	...
	t.Run("ed25519 raw hex", func(t *testing.T) {
		...
		expected := strings.ReplaceAll(edFP, ":", "")
		if got != expected {
...
func TestPrivateKeySignerEd25519OpenSSH(t *testing.T) {
	priv, pemBytes, fingerprint := generateEd25519OpenSSHKey(t)
...
func TestFormatPublicKeyFingerprintEd25519Pointer(t *testing.T) {
	_, priv, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("ed25519.GenerateKey: %v", err)
	}

	// Value type — existing behavior.
	fpValue, err := formatPublicKeyFingerprint(priv, true)
	if err != nil {
		t.Fatalf("value type: %v", err)
	}

	// Pointer type — the fix for OpenSSH-format keys.
	fpPointer, err := formatPublicKeyFingerprint(&priv, true)
	if err != nil {
		t.Fatalf("pointer type: %v", err)
	}

	if fpValue != fpPointer {
		t.Errorf("pointer fingerprint %q != value fingerprint %q", fpPointer, fpValue)
	}
}
...
func TestNewSSHAgentSigner_ClosesConnOnFailure(t *testing.T) {
	...
	go func() {
		...
		// ServeAgent returns io.EOF when the client disconnects, which
		// is expected in this test.
		if err := agent.ServeAgent(agent.NewKeyring(), conn); err != nil && err != io.EOF {
			t.Errorf("agent.ServeAgent: %v", err)
		}
