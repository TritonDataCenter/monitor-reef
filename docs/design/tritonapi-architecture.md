...
Tracked in beads. Pick these up before deploying to a real cluster.

### Gateway correctness

- **monitor-reef-skmk** -- Host header not rewritten when proxying. Client's
  Host passes through to the backend, which is wrong for CloudAPI's
  virtual-host routing and for `127.0.0.1:8080` loopback.
- **monitor-reef-zi0q** -- No `X-Forwarded-For` / `Forwarded` headers.
  CloudAPI audit logs record the gateway's IP as every caller.
- **monitor-reef-sexj** -- WebSocket proxying not implemented. CloudAPI's
  three WebSocket endpoints will silently degrade to HTTP.
- **monitor-reef-hmqm** -- URL path joining can produce `//`. Backends may
  route this incorrectly.

### Gateway/service ops

- **monitor-reef-qofq** -- SMF dependency `restart_on="restart"` should be
  `"none"`. A triton-api restart should not cascade to the gateway.
- **monitor-reef-5011** -- Missing `config-agent` SMF dependency. Services
  can start before configs are rendered.
- **monitor-reef-5iuq** -- Gateway runs as root. Should drop to `nobody`
  with `net_privaddr` (mariana-trench pattern).
- **monitor-reef-22nv** -- Config loading silently falls back to defaults
  on parse/IO error. Should fail fast so SMF marks the service in
  maintenance.
- **monitor-reef-b26a** -- No request-ID generation or propagation. Retrofit
  later is expensive; add from day one.
- **monitor-reef-m3o9** -- No graceful shutdown on SIGTERM. In-flight
  requests are dropped on `svcadm` stop.

### triton-api-server

- **monitor-reef-0iqe** -- Body size limit of 1 MiB is too small for
  CloudAPI compat. Raise and make configurable via SAPI.
