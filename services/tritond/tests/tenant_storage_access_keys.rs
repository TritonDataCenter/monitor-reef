...
//! (monitor-reef-8imp). Same shape as `tenant_storage_buckets.rs`
//! but for the nested per-user resource. The user path-segment is a
//! free-form string (mantad's IAM scheme) — when the user doesn't
//! exist in the tenant's workspace mantad's gate produces a 404; we
//! exercise the handler-owned gates only.
