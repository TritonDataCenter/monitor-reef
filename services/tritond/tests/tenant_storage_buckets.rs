...
//! `GET /v1/silos/{silo_id}/tenants/{tenant_id}/storage/buckets`
//! (monitor-reef-1ojf). Covers the gates the new handler owns —
//! authn/authz, cross-silo defence, 412 on an unbound tenant, and
//! the upstream-error pass-through against an unreachable mantad.
//! The actual bucket-filtering correctness lives in mantad (the
//! `?workspace=` query has its own tests in `mantas3/cluster`);
//! tritond's job here is only to derive the workspace name from the
//! tenant binding and forward it.
...
impl TestServer {
    /// Spin up tritond with a root operator (drives all setup) and a
    /// tenantless non-root user (used for the "non-root denied" gate).
    /// Keep a handle to the store so the test can synthesise tenant
    /// storage bindings directly, sidestepping the init-storage flow
    /// (which requires a reachable mantad to mint the workspace).
    async fn start() -> Self {
...
/// Build an unreachable-cluster registration so forwarder calls fail
/// fast with `ECONNREFUSED` instead of stalling on connect.
...
async fn anonymous_is_denied() {
    // Silo-scoped auth (`authenticate_and_authorize_in_silo` +
    // Cedar's silo-member rule) deliberately surfaces "denied" as
    // 404 rather than 401/403 so an unauthenticated probe cannot
    // distinguish a real silo from a missing one. Accept either
    // 401, 403, or 404 here so a future tightening of the
    // anonymous gate doesn't break the test.
...
async fn non_root_in_silo_is_forbidden() {
    // The new endpoint is silo-scoped via `Action::TenantGet`. A
    // tenantless non-root principal does not satisfy the
    // silo-membership rule and should land as 403/404 (the Cedar
    // resource is silo-rooted, so a 403 is acceptable here — the
    // cross-silo defence comes from the *tenant* lookup, not the
    // silo-level gate).
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
...
async fn unbound_tenant_returns_412_tenant_storage_unbound() {
    ...
    // No storage binding has been written, so the handler must trip
    // the same precondition the binding lifecycle uses elsewhere.
    let err = root
        .list_silo_tenant_storage_buckets()
        .silo_id(silo_id)
        .tenant_id(tenant_id)
...
async fn cross_silo_probe_returns_404() {
    ...
    // Probing tenant-a through silo-b's URL must surface as 404 (not
    // 403 / 200) so the response cannot enumerate tenants in other
    // silos.
    let err = root
        .list_silo_tenant_storage_buckets()
        .silo_id(silo_b)
...
async fn bound_tenant_against_unreachable_mantad_surfaces_5xx() {
    // With a tenant bound to a cluster pointing at a dead port, the
    // forwarder layer (`client.list_buckets`) fails on connect. The
    // mapping layer must surface that as 5xx, never 4xx — the
    // failure is upstream, not the operator's fault. This exercises
    // the happy-path branches of the handler (auth ok, cross-silo
    // ok, binding present, cluster resolved) before the upstream
    // error.
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    ...
    // Synthesise the binding directly: a real init-storage call
    // would round-trip through mantad's `create_workspace`, which we
    // do not have a fixture for.
