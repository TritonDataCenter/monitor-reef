...
//! End-to-end tests for the tenant-scoped user write endpoints
//! (monitor-reef-5fek, Wave 2):
//!
//!   POST   /v1/silos/{sid}/tenants/{tid}/storage/users
//!   DELETE /v1/silos/{sid}/tenants/{tid}/storage/users/{user}
//!
//! Same shape as the Wave 1 `tenant_storage_users.rs` read-side file.
//! Covers the handler-owned gates only — authn/authz, cross-silo
//! defence, 412 on an unbound tenant, and upstream-error pass-through
//! against an unreachable mantad. Workspace-resolution correctness
//! (i.e. the user landing in `t-<tenant_uuid_simple>` rather than
...
use tritond_client::types::{NewSilo, NewTenant, StorageCreateUserRequest};
...
fn user_body(name: &str) -> StorageCreateUserRequest {
    StorageCreateUserRequest {
        name: name.to_string(),
    }
}
...
async fn create_anonymous_is_denied() {
    ...
    let err = anon
        .create_silo_tenant_storage_user()
        .silo_id(silo_id)
        .tenant_id(tenant_id)
        .body(user_body("alice"))
        .send()
        .await
