...
//! End-to-end tests for the tenant-scoped policies endpoints
//! (monitor-reef-fydj):
//!
//!   GET /v1/silos/{sid}/tenants/{tid}/storage/users/{u}/policies
//!   GET /v1/silos/{sid}/tenants/{tid}/storage/users/{u}/policies/{p}
//!
//! Same shape as the other tenant_storage_* test files. Policy
//! bodies (raw JSON) are mantad-owned; tritond's gates are the only
...
#[tokio::test]
async fn list_anonymous_is_denied() {
...
#[tokio::test]
async fn list_non_root_in_silo_is_forbidden() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
...
#[tokio::test]
async fn list_unbound_tenant_returns_412_tenant_storage_unbound() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
    ...
    assert_eq!(body.error_code.as_deref(), Some("TenantStorageUnbound"));

    test.close().await;
}

#[tokio::test]
async fn list_cross_silo_probe_returns_404() {
    let test = TestServer::start().await;
    let root = test.root_client();
...
#[tokio::test]
async fn list_bound_tenant_against_unreachable_mantad_surfaces_5xx() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
...
// The single-policy GET endpoint shares the same gates; one
// representative test confirms the cross-silo defence on the
// detail-view path (where the leak would be most sensitive — the
// raw policy body).
#[tokio::test]
async fn get_cross_silo_probe_returns_404() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_a = make_silo(&root, "silo-a").await;
    let silo_b = make_silo(&root, "silo-b").await;
...
#[tokio::test]
async fn get_unbound_tenant_returns_412_tenant_storage_unbound() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
