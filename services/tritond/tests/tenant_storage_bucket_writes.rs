...
//! End-to-end tests for the tenant-scoped bucket write endpoints
//! (monitor-reef-5fek, Wave 2):
//!
//!   POST   /v1/silos/{sid}/tenants/{tid}/storage/buckets
//!   DELETE /v1/silos/{sid}/tenants/{tid}/storage/buckets/{bucket}
//!
//! Same shape as the Wave 1 `tenant_storage_buckets.rs` read-side
//! file. Covers the handler-owned gates only — authn/authz,
//! cross-silo defence, 412 on an unbound tenant, and upstream-error
//! pass-through against an unreachable mantad. The actual workspace
//! isolation (a bucket landing in `t-<tenant_uuid_simple>` and not
//! cluster-root) is mantad's responsibility, exercised in the
//! `mantas3/cluster` tests.
...
use tritond_client::types::{NewSilo, NewTenant, StorageCreateBucketRequest};
...
fn bucket_body(name: &str) -> StorageCreateBucketRequest {
...
async fn create_anonymous_is_denied() {
    ...
    let err = anon
        .create_silo_tenant_storage_bucket()
        .silo_id(silo_id)
        .tenant_id(tenant_id)
        .body(bucket_body("bucket-a"))
        .send()
        .await
