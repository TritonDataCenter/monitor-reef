...
use tritond_client::types::{
    AddressFamily, NewFloatingIp, NewIdpConfig, NewNatGateway, NewProject, NewSilo, NewVpc,
};
use tritond_store::{MemStore, Store, User};
use uuid::Uuid;

const ROOT_PASSWORD: &str = "nat-gateway-test";
...
async fn make_project_vpc(root: &tritond_client::Client, silo_name: &str) -> (Uuid, Uuid, Uuid) {
    let silo = root
        .create_silo()
        .body(NewSilo {
...
async fn root_can_create_list_get_and_delete_nat_gateway() {
    ...
    assert_eq!(nat.public_address, IpAddr::V4(want));
    assert_eq!(nat.desired_generation, 1);
    assert_eq!(nat.realized.desired_generation, 1);
    assert_eq!(nat.realized.applied_generation, None);
    ...
    let err = root
        ...
        .send()
        .await
        .expect_err("deleted nat gateway must 404");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn nat_gateway_shares_public_pool_with_floating_ips() {
    let test = TestServer::start().await;
    let root = test.root_client();
    ...
    assert_eq!(fip.address, IpAddr::V4("203.0.113.2".parse().unwrap()));
    assert_eq!(
        nat.public_address,
        IpAddr::V4("203.0.113.3".parse().unwrap())
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn duplicate_nat_gateway_name_within_vpc_returns_409() {
    let test = TestServer::start().await;
    let root = test.root_client();
    ...
    let err = root
        ...
        .send()
        .await
        .expect_err("duplicate nat gateway name must 409");
...
async fn cross_vpc_get_and_delete_return_404() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let (tenant_id, project_id, vpc_a) = make_project_vpc(&root, "cross").await;
    let vpc_b = root
        .create_vpc_v1()
        .tenant(tenant_id)
        .project(project_id)
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn federated_user_can_manage_nat_gateways_in_own_tenant_only() {
    let test = TestServer::start().await;
    let (idp, shutdown) = StubIdp::start().await;
    let root = test.root_client();
    let (alpha_tenant, alpha_project, alpha_vpc) = make_project_vpc(&root, "alpha").await;
    let (beta_tenant, beta_project, beta_vpc) = make_project_vpc(&root, "beta").await;
    ...
    let nat = tenant
        ...
        .send()
        .await
        .expect("alpha tenant should create nat gateway in alpha")
        .into_inner();
    assert_eq!(nat.tenant_id, alpha_tenant);
    ...
    let err = tenant
        ...
        .send()
        .await
        .expect_err("alpha tenant must not create beta nat gateway");
    assert_status(err, 404);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn anonymous_cannot_reach_nat_gateway_endpoints() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let (tenant_id, project_id, vpc_id) = make_project_vpc(&root, "anon").await;
