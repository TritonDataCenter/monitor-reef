...
//! End-to-end tests for the tenant-scoped policy PUT endpoint
//! (monitor-reef-5fek, Wave 2):
//!
//!   PUT /v1/silos/{sid}/tenants/{tid}/storage/users/{user}/policies/{policy}
//!
//! Same shape as the Wave 1 `tenant_storage_policies.rs` read-side
//! file. Covers the handler-owned gates only — authn/authz,
//! cross-silo defence, 412 on an unbound tenant, and upstream-error
//! pass-through against an unreachable mantad. The policy body is
//! opaque `serde_json::Value` (mantad owns the IAM policy schema), so
//! we just send a syntactically-valid-looking document.
...
#[tokio::test]
async fn put_anonymous_is_denied() {
...
#[tokio::test]
async fn put_non_root_in_silo_is_forbidden() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
...
#[tokio::test]
async fn put_unbound_tenant_returns_412_tenant_storage_unbound() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
...
#[tokio::test]
async fn put_cross_silo_probe_returns_404() {
    let test = TestServer::start().await;
    let root = test.root_client();
...
#[tokio::test]
async fn put_bound_tenant_against_unreachable_mantad_surfaces_5xx() {
    let test = TestServer::start().await;
    let root = test.root_client();
    let silo_id = make_silo(&root, "ops").await;
    let tenant_id = make_tenant(&root, silo_id, "acme").await;
