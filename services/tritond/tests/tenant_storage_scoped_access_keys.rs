...
//! End-to-end tests for the tenant-scoped bucket-scoped access-key
//! mint endpoint (monitor-reef-y6n2):
//!
//!   POST /v1/silos/{sid}/tenants/{tid}/storage/users/{user}/scoped-access-keys
//!
//! Same shape as the Wave 2 `tenant_storage_access_key_writes.rs`
...
//! against an unreachable mantad. The per-bucket level enforcement is
//! mantad's job and is covered cross-owner in
//! `manta-storage/crates/mantas3/s3/tests/scoped_keys_e2e.rs`.
...
use tritond_client::types::{
    NewSilo, NewTenant, StorageScopeEntry, StorageScopeLevel, StorageScopedAccessKeyRequest,
};
...
fn sample_scope() -> StorageScopedAccessKeyRequest {
    StorageScopedAccessKeyRequest {
        scope: vec![StorageScopeEntry {
            bucket: "irrelevant-bucket".to_string(),
            level: StorageScopeLevel::Read,
            key_prefix: None,
        }],
    }
}
...
async fn unbound_tenant_returns_412_tenant_storage_unbound() {
    ...
    assert_eq!(
        ...
        "412 must use the same error code as the rest of the tenant-scoped family"
    );
