...
pub async fn register_or_resume(
    ...
    // The CN's nic_tag inventory is published *after* registration, on
    // the authenticated `POST /v1/agent/nic-tags` endpoint (see
    // `tritonagent::run`), so the write is keyed by the authenticated
    // CN rather than a spoofable `server_uuid` on this anonymous path.
