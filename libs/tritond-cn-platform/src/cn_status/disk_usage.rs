...
//!    pluggable [`ImageDatasetFilter`] trait so the consumer decides which
//!    `<pool>/<uuid>` datasets count as "real" image stores).
...
//!    totals.
//!
//! Keeping the field names identical means consumers stay oblivious to the
//! rewrite.
...
/// Disk-accounting fields posted to the control plane.
///
...
/// Filters dataset names that the caller considers "installed images"
/// for the purposes of disk-usage accounting. Pluggable so consumers
/// can wire in whatever image-store layout they use (legacy imgadm
/// manifests, our content-addressed `zones/<uuid>` layout, etc.).
pub trait ImageDatasetFilter: Send + Sync + 'static {
    fn is_image_dataset(&self, dataset_name: &str) -> bool;
}

/// `ImageDatasetFilter` that accepts every dataset. Useful for tests.
#[derive(Debug, Clone, Copy, Default)]
pub struct AcceptAllImageFilter;

impl ImageDatasetFilter for AcceptAllImageFilter {
    fn is_image_dataset(&self, _dataset_name: &str) -> bool {
        true
    }
}

/// `ImageDatasetFilter` that returns true when the basename of the dataset
/// (after the pool prefix) parses as a UUID. Matches our content-addressed
/// `zones/<uuid>` layout where every top-level child of the pool whose name
/// is a UUID is treated as an installed image.
#[derive(Debug, Clone, Copy, Default)]
pub struct UuidNamedImageFilter;

impl ImageDatasetFilter for UuidNamedImageFilter {
    fn is_image_dataset(&self, dataset_name: &str) -> bool {
        is_image_dataset_name(dataset_name).is_some()
    }
}

/// Pattern: `<pool>/<uuid>` with a valid v4-ish UUID. We don't pull in the
/// `regex` crate; this plain-Rust helper does the same job and stays in
/// sync with the legacy heuristic.
...
pub struct DiskUsageSampler {
    zfs: Arc<ZfsTool>,
    filter: Arc<dyn ImageDatasetFilter>,
...
impl DiskUsageSampler {
    pub fn new(zfs: Arc<ZfsTool>, filter: Arc<dyn ImageDatasetFilter>) -> Self {
        Self {
            zfs,
            filter,
    ...
    pub async fn sample(&self, vms: &[VmSnapshot]) -> Result<DiskUsage, DiskUsageError> {
        ...
        self.account_installed_images(&mut usage, &datasets);
    ...
    /// Walk every dataset; if the configured [`ImageDatasetFilter`] flags
    /// it as an installed image, credit its bytes to
    /// `installed_images_used_bytes`.
    fn account_installed_images(
        &self,
        usage: &mut DiskUsage,
        datasets: &serde_json::Map<String, serde_json::Value>,
    ) {
        for (name, ds) in datasets {
            if !self.filter.is_image_dataset(name) {
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn uuid_named_filter_matches_uuid_basenames() {
        let filter = UuidNamedImageFilter;
        assert!(filter.is_image_dataset("zones/5135b4bb-da9e-48e2-8965-7424267ad23e"));
        assert!(!filter.is_image_dataset("zones/cores"));
        assert!(!filter.is_image_dataset("zones/5135b4bb-da9e-48e2-8965-7424267ad23e/root"));
    }

    #[test]
    fn accept_all_filter_takes_everything() {
        let filter = AcceptAllImageFilter;
        assert!(filter.is_image_dataset("anything"));
        assert!(filter.is_image_dataset("zones/cores"));
    }
    ...
    async fn sample_computes_zones_and_pool_totals() {
        // Build a sampler that can't actually read zfs -- we drive it via
        // a DiskUsage computed by hand. This is a narrow unit test for the
        // non-async accounting halves.
        let zfs = Arc::new(ZfsTool::with_bins("/nonexistent/zfs", "/nonexistent/zpool"));
        let filter: Arc<dyn ImageDatasetFilter> = Arc::new(UuidNamedImageFilter);
        let sampler = DiskUsageSampler::new(zfs, filter);
    ...
    #[test]
    fn account_installed_images_credits_only_filtered_datasets() {
        let zfs = Arc::new(ZfsTool::with_bins("/nonexistent/zfs", "/nonexistent/zpool"));
        let filter: Arc<dyn ImageDatasetFilter> = Arc::new(UuidNamedImageFilter);
        let sampler = DiskUsageSampler::new(zfs, filter);
        ...
        let mut datasets = serde_json::Map::new();
        datasets.insert(
            "zones/5135b4bb-da9e-48e2-8965-7424267ad23e".to_string(),
            serde_json::json!({"used": "777"}),
        );
        datasets.insert(
            "zones/cores".to_string(),
            serde_json::json!({"used": "999"}),
        );

        let mut usage = DiskUsage::default();
        sampler.account_installed_images(&mut usage, &datasets);
        assert_eq!(usage.installed_images_used_bytes, 777);
    }
}
