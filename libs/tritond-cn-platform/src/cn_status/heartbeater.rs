...
//! Heartbeater: posts periodic liveness, status, and sysinfo updates via
//! the [`StatusSink`] trait.
//!
//! The legacy implementation used a mix of `setInterval`s and a vasync queue
//! that serialized updates so the upstream control plane never saw more
//! than one request at a time per compute node. The Rust port keeps that
//! serialization via a single `tokio::select!` loop in [`Heartbeater::run`]:
//! only one task ever holds the [`StatusSink`] at a time.
...
use crate::cn_status::collector::StatusCollector;
use crate::cn_status::watchers::DirtyFlag;
...
/// Transport-shaped error returned by [`StatusSink`] implementations.
///
/// Kept opaque so the heartbeater doesn't need to know whether the
/// underlying transport is HTTP, an in-process channel, or something else.
#[derive(Debug, Error)]
#[non_exhaustive]
...
/// Pluggable sink for heartbeats and status payloads.
///
/// The actual transport (calls to tritond, CNAPI, or whatever the
/// embedding service uses) is supplied by the consumer. Implementations
/// must be cheap to call from the heartbeater loop -- failures are logged
/// and the loop continues without retrying within the same tick.
#[async_trait::async_trait]
pub trait StatusSink: Send + Sync + 'static {
    /// Called every `heartbeat_interval` (default 5s).
    async fn post_heartbeat(&self) -> Result<(), StatusSinkError>;
    /// Called when a status sample has been collected.
...
impl HeartbeaterHandle {
    ...
    /// survives even if the heartbeater task is currently mid-request and
    /// hasn't re-entered the `select!` yet.
...
///
/// Holds the pieces it needs to publish: a [`StatusSink`], a status
/// collector, a shared [`DirtyFlag`] that watchers can poke to force a
/// status sample, and timing knobs. Built via [`Heartbeater::new`], run via
...
impl Heartbeater {
    pub fn new(sink: Arc<dyn StatusSink>, collector: StatusCollector) -> Self {
        Self {
            sink,
            collector,
            dirty: DirtyFlag::new(),
    ...
    ///
    /// * `heartbeat_interval` (default 5s) -- post a heartbeat.
    /// * `status_max_interval` (default 60s) -- force a status sample even
    ///   if nothing has signaled dirty, so the consumer never sees stale
    ///   data longer than a minute.
    /// * `status_check_interval` (default 500ms) -- check the dirty flag;
    ///   if any watcher has raised it since the last sample, take a new one.
    ///
    ...
    pub async fn run(self, shutdown: Arc<Notify>) {
        ...
        // Mark dirty once at startup so the consumer gets a first status
        // sample well before the 60s `status_max_interval` has a chance to
        // fire.
        self.dirty.mark();
        ...
        loop {
            tokio::select! {
                ...
                _ = heartbeat_tick.tick() => {
                    if let Err(e) = self.sink.post_heartbeat().await {
    ...
    async fn collect_and_post(&self) {
        ...
        if let Err(e) = self.sink.post_status(&body).await {
