...
#[test]
#[ignore = "upstream test depends on a host-specific fixture path that does not exist in this fork"]
fn parse() {
    use std::io::Read;
    use std::io::Seek;
    ...
    for snapshot in qcow.snapshots() {
        println!(
            "Snapshot {:?}: {:?} (size = {})",
            snapshot.unique_id, snapshot.name, snapshot.vm_state_size
        );
    }
    ...
    let _ = qcow.l1_table[0].read_l2(&mut file, qcow.header.cluster_bits);
    ...
    std::io::copy(&mut reader.take(0x400_0000), &mut std::io::sink()).unwrap();
}
