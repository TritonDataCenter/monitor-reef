...
use backoff::{Error as BackoffError, ExponentialBackoff, retry};
    ...
    F: FnMut(&Backend) -> C + Send,
{
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        ...
        let _ = debug_trait_builder.field("protected_data", &&(protected_data));
        let _ =
            debug_trait_builder.field("resolver_thread", &&(resolver_thread));
        let _ = debug_trait_builder
            .field("resolver_rx_thread", &&(resolver_rx_thread));
        let _ = debug_trait_builder.field("resolver_tx", &&(resolver_tx));
        let _ =
            debug_trait_builder.field("max_connections", &&(max_connections));
        let _ = debug_trait_builder.field("claim_timeout", &&(claim_timeout));
        let _ =
            debug_trait_builder.field("rebalance_check", &&(rebalance_check));
        let _ =
            debug_trait_builder.field("rebalance_thread", &&(rebalance_thread));
        let _ =
            debug_trait_builder.field("rebalancer_stop", &&(rebalancer_stop));
        let _ = debug_trait_builder
    ...
    C: Connection,
{
    ...
    let mut connection_data = protected_data.connection_data_lock();
    shuffle(&mut connection_data.connections, rand::rng(), log.clone());
}
    ...
    R: rand::Rng,
{
    ...
    while i > 1 {
        i -= 1;
        let new_idx = rng.random_range(0..i);
        debug!(
            log,
