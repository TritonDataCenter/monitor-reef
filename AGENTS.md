...
### 3. build.rs Patch Consistency

Every enum used as a CLI argument must also have `with_patch(EnumName, &value_enum_patch)` in the corresponding client's `build.rs`. This ensures the Progenitor-generated copy also gets `ValueEnum` for cases where the CLI uses `types::EnumName`.
