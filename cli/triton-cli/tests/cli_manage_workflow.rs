...
/// Test `triton instance resize` emits a GET for the package then a POST resize action
#[test]
fn test_resize_payload() {
    ...
    // Should have a GET for the package verification
    let get = envelopes
        .iter()
        .find(|e| e["method"] == "GET")
        .expect("should have a GET envelope for package verification");

    let get_path = get["path"].as_str().expect("path should be a string");
    assert!(
        get_path.contains("/packages/00000000-0000-0000-0000-000000000002"),
        "GET path should be for the package, got: {get_path}"
    );

    // Should have a POST for the resize action
    let post = envelopes
        .iter()
        .find(|e| e["method"] == "POST")
        .expect("should have a POST envelope for resize");

    let post_path = post["path"].as_str().expect("path should be a string");
    assert!(
        post_path.contains("/machines/00000000-0000-0000-0000-000000000003"),
        "POST path should be for the machine, got: {post_path}"
    );

    assert_eq!(
        post["body"]["action"], "resize",
        "POST body should have action=resize"
    );
}
