...
//! IMGAPI CLI - Command-line interface for Triton IMGAPI
//!
//! This CLI provides access to all IMGAPI endpoints for managing images
//! in a Triton datacenter.
//!
//! # Environment Variables
//!
//! - `IMGAPI_URL` - IMGAPI base URL (default: http://localhost)
...
use imgapi_client::{Client, TypedClient};
use imgapi_client::{
...
#[derive(Parser)]
#[command(name = "imgapi", version, about = "CLI for Triton IMGAPI")]
struct Cli {
    /// IMGAPI base URL
    #[arg(long, env = "IMGAPI_URL", default_value = "http://localhost")]
    base_url: String,
...
fn print_image_summary(image: &types::Image) {
    ...
    println!("State: {}", enum_to_display(&image.state));
    if let Some(ref os) = image.os {
        println!("OS: {}", enum_to_display(os));
    }
    if let Some(ref t) = image.type_ {
        println!("Type: {}", enum_to_display(t));
    }
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::AdminGetState => {
            let resp = client.admin_get_state().send().await?;
            println!("{}", serde_json::to_string_pretty(&resp.into_inner())?);
        }

        Commands::AdminDropCaches => {
            typed_client
                .drop_caches()
                .await
        ...
        Commands::ListImages {
            ...
            if raw {
                ...
                for img in &images {
                    let type_str = img.type_.as_ref().map(enum_to_display).unwrap_or_default();
                    println!(
                        "{}: {} v{} ({}, {})",
                        img.uuid,
                        img.name,
                        img.version,
                        enum_to_display(&img.state),
                        type_str,
                    );
                }
        ...
        Commands::ChannelAddImage { uuid, channel } => {
            let image = typed_client
        ...
        // ACL Management
        // ====================================================================
        Commands::AddImageAcl { uuid, acl_uuids } => {
        ...
        Commands::RemoveImageAcl { uuid, acl_uuids } => {
            let uuids = parse_uuid_list(&acl_uuids)?;
            let image = typed_client
        ...
        // Admin Push
        // ====================================================================
        Commands::AdminPushImage { uuid, channel } => {
