...
//! `tritonadm sapi <subcommand>` — thin wrapper over the SAPI client, covering
//! the full SAPI HTTP API (applications, services, instances, manifests, mode,
//! log level, cache, configs). Absorbed from the standalone `sapi-cli` so that
//! admin tasks live in one binary.
...
impl SapiCommand {
    pub async fn run(self, sapi_url: &str) -> Result<()> {
        ...
        match self {
            ...
            SapiCommand::SetLogLevel { level, raw: _ } => {
            ...
            SapiCommand::GetApplication { uuid, raw } => {
                let resp = client
                    .get_application()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::DeleteApplication { uuid } => {
                client
                    .delete_application()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::GetService { uuid, raw } => {
                let resp = client
                    .get_service()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::DeleteService { uuid } => {
                client
                    .delete_service()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::GetInstance { uuid, raw } => {
                let resp = client
                    .get_instance()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::DeleteInstance { uuid } => {
                client
                    .delete_instance()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::GetInstancePayload { uuid, raw: _ } => {
                let resp = client
            ...
            SapiCommand::GetManifest { uuid, raw } => {
                let resp = client
                    .get_manifest()
                    .uuid(uuid)
                    .send()
            ...
            SapiCommand::DeleteManifest { uuid } => {
                client
                    .delete_manifest()
                    .uuid(uuid)
                    .send()
            ...
            // Configs
            // ================================================================
            SapiCommand::GetConfig { uuid, raw: _ } => {
                let resp = client
                    .get_config()
                    .uuid(uuid)
                    .send()
